...
public:
  enum EnvType { None, Normal, Octa, Pit, BoxRing, Uterus, Stacked };
    ...
    case Normal:
      {
        ...
        Substance substance(roughness, 0.005, hardness, 0.95);
      ...
      if (type==BoxRing) { // this playground comes second for dropping obstacles
        playground = new Playground(odeHandle, osgHandle,
                                    osg::Vec3(pitsize, 0.20, heightInner)); 
        //     playground->setTexture("Images/really_white.rgb");
        //        playground->setGroundTexture("Images/yellow_velour.rgb");
        if(useColorSchema)
          playground->setTexture(0,0,TextureDescr("Images/wall_bw.jpg",-1.5,-3)); // was: wall.rgb 
        playground->setGroundThickness(0.2);
        playground->setPosition(pitPosition);
        Substance substance(3, 0.005, hardness, 0.95);
        playground->setGroundSubstance(substance);
        global.obstacles.push_back(playground);
        playgrounds.push_back(playground);
      }
  ...
  void placeObstacles(const OdeHandle& odeHandle, const OsgHandle& osgHandle, 
    ...
    for(int i=0; i<numSeeSaws; i++){
      ...
      seesaw->setPose(ROTM(M_PI/2.0,0,0,1)*TRANSM(4, -i,.2));
      global.obstacles.push_back(seesaw);
    }

    for(int i=0; i<numBoxPiles && i<2; i++){
      double h = 0.4-0.2*i;
      double s = 5.5-1.5*i;
      double b = 1.1-0.2*i;
      Boxpile* boxpile = new Boxpile(odeHandle, osgHandle,
                                     osg::Vec3(s, s, 1.0), 30-5*i, 1+2*i,
                                     osg::Vec3(b, b, h),// mean 
                                     osg::Vec3(0.5, 0.5, h/2) // var
                                     );

      boxpile->setColor("wall"); 
      boxpile->setPose(ROTM(M_PI/5.0*i,0,0,1)
                       * TRANSM(widthground/3-2.0/3.0*widthground*i, 
                                -widthground/3-1.0*i, 0.2));
	...
	new PassiveSphere(odeHandle, osgHandle.changeColor("Monaco"), 0.2);
      s->setPosition(Pos(i*0.5-2, 3+i*1, 1.0)); 
	...
	new PassiveBox(odeHandle, osgHandle.changeColor("Weissgrau"), 
      ...
      b->setTexture(TextureDescr("Images/light_chess.rgb",1,1));
