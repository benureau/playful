  ...
  { 
    ...
    if(conf.useVelocityServos){
      ...
      addParameter("dampingfactor",  &conf.dampingFactor,0,10, 
                   "global damping factor for all motor");
    }
...
//       }
  ...
  void Rhoenrad::notifyOnChange(const paramkey& key){    
    // if fixated to wheel then no power
    double powerFactor = hip_wheel ? 0 : conf.powerFactor;

    // we just set all parameters independend of what was actually changed
    FOREACH(vector<TwoAxisServo*>, hipservos, i){
      if(*i) { 
	(*i)->setPower( conf.hipPower * powerFactor, conf.hip2Power * powerFactor);
    ...
    FOREACH(vector<TwoAxisServo*>, headservos, i){
      if(*i){
	(*i)->setPower(conf.neckPower * powerFactor, conf.neckPower * powerFactor);
    ...
    FOREACH(vector<OneAxisServo*>, kneeservos, i){
      if(*i){
	(*i)->setPower(conf.kneePower * powerFactor);
    ...
    FOREACH(vector<OneAxisServo*>, ankleservos, i){
      if(*i){
	(*i)->setPower(conf.anklePower * powerFactor);
    ...
    FOREACH(vector<TwoAxisServo*>, armservos, i){
      if(*i){
	(*i)->setPower(conf.armPower * powerFactor * conf.relForce, 
                       conf.armPower * powerFactor * conf.relForce);
...
//	(*i)->setMinMax1(-conf.armJointLimit, conf.armJointLimit);
    ...
    FOREACH(vector<OneAxisServo*>, arm1servos, i){
      if(*i){
	(*i)->setPower(conf.elbowPower * powerFactor * conf.relForce);
    ...
    pelvisservo->setPower(conf.pelvisPower * powerFactor); 
    ...
    FOREACH(vector<OneAxisServo*>, backservos, i){
      if(*i){
	(*i)->setPower(conf.backPower * powerFactor);
